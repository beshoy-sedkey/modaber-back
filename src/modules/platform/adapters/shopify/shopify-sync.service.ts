...
export class ShopifySyncService {
  ...
  async syncAllProducts(merchantId: string): Promise<{ synced: number }> {
    ...
    this.logger.log(`Synced ${totalSynced} products for merchant: ${merchantId}`);
  ...
  async scheduleSyncForAllMerchants(): Promise<void> {
    const merchants = await this.prisma.merchant.findMany({
      where: { isActive: true, platformType: PlatformType.shopify },
      select: { id: true },
    });
    ...
    for (const merchant of merchants) {
      await this.syncQueue.add(
        ...
        { merchantId: merchant.id },
        {
          ...
          jobId: `shopify-sync-${merchant.id}`,
        },
      );
    }

    this.logger.log(`Scheduled sync for ${merchants.length} Shopify merchants`);
  }
}
